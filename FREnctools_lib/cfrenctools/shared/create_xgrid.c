...
#define MAX_V 8
  ...
  // set up blocks for openmp threading (if using) and allocate passed in arrays
  block_setup(i_in, j_in, i_out, j_out, xgrid_area,
              &pi_in, &pj_in, &pi_out, &pj_out, &pxgrid_area,
              &istart2, &iend2, &pstart, &pnxgrid, nx2, ny2);

  // allocate arrays used in the search
  ...
  // find the bounds for each grid cell (ie. min/max) 
#if defined(_OPENMP)
...
int inside_edge(double x0, double y0, double x1, double y1, double x, double y)
{
 ...
 /**
  * Partitions data for openmp threading, used in create_xgrid_2dx2d order1&2
  * 
  ...
  */
void block_setup(int const* i_in, int const* j_in, int const* i_out, int const* j_out,
